...
# --- Session State ---
if "llm_no_rag" not in st.session_state:
    st.session_state.llm_no_rag = None
if "llm_rag" not in st.session_state:
    st.session_state.llm_rag = None
...
def run_rag_query(query, hf_token, temperature):
    ...
    if not hf_token:
        st.warning("Por favor, ingresa tu Secret de Hugging Face para usar RAG.")
        return None
...
def run_no_rag_query(query, hf_token, temperature):
    ...
    if not hf_token:
        st.warning("Por favor, ingresa tu Secret de Hugging Face para usar el modelo.")
        return None
...
if st.button("Obtener Respuesta"):
    if hf_token:
        st.session_state.llm_no_rag = run_no_rag_query(user_question, hf_token, temperature)
        st.session_state.llm_rag = run_rag_query(user_question, hf_token, temperature)
    else:
...
if st.session_state.llm_no_rag and st.session_state.llm_rag:
